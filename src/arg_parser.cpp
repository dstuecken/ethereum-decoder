#include "ethereum_decoder/arg_parser.h"
#include <iostream>
#include <stdexcept>
...
void ArgParser::printUsage(const char* programName) const {
    ...
    std::cerr << "  " << programName << " abis/erc20.json --log-data \"topics:data\" --format json" << std::endl;
}

bool ArgParser::isFlag(const std::string& arg) const {
    return arg.length() >= 2 && arg.substr(0, 2) == "--";
}

OutputFormat ArgParser::parseOutputFormat(const std::string& format) const {
    if (format == "human") {
