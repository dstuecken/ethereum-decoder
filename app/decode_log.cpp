...
#include "ethereum_decoder/log_data.h"
    ...
    try {
        ...
        switch (args.logSource) {
            case LogSource::LOG_DATA: {
                LogEntry customLog = LogData::parse(args.logData);
        ...
        if (args.outputFormat == OutputFormat::JSON) {
            ...
            if (decodedLogs.size() == 1) {
                result = JsonDecoder::decodedLogToJson(*decodedLogs[0]);
            } else {
                result = nlohmann::json::array();
                for (const auto& decodedLog : decodedLogs) {
                    result.push_back(JsonDecoder::decodedLogToJson(*decodedLog));
                }
            }
